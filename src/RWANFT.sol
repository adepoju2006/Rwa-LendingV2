...
contract DCCRWANFT is ERC721, ERC721Enumerable, Ownable {
    ...
    constructor(address _token, address _housePrices, address initialOwner) ERC721("DCC_RWA NFT", "DCCNFT") Ownable(initialOwner) {
        if (_token == address(0) || _housePrices == address(0) || initialOwner == address(0)) revert RWANFT__InvalidAddress();
        token = IERC20(_token);
